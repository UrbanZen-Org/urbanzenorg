	...
	Zoomer.prototype = {
		...
		***********************************************************************************************************************/
		init: function () {
			...
			if (supportsTouch) {
				$image.bind(self.event_down, function (e) {
					e.preventDefault();							
				});
			}
			...
			if ((self.mouse_drag && !supportsTouch) || (self.touch_drag && supportsTouch)) {				
				if (supportsTouch) {
					self.$hitArea.bind(self.event_down, {self: self}, self.mouseDown);
			...
			if (self.mouse_dbl_click) {
				...
				(supportsTouch? self.$hitArea : self.$holder).bind('dblclick.sz', function (e) {					
		...
		***********************************************************************************************************************/
		setContainer: function () {			
			...
			self.$hitArea = $('<div style="position: absolute; z-index: '+(supportsTouch? 2 : 1)+'; top: 0px; left: 0px; width: 100%; height: 100%;" ></div>').appendTo($holder);
